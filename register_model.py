...
def run(experiment_name, model_name):
    ...
    best_run = runs[0]
    print(f"Best run: {best_run.info.run_id} {best_run.data.metrics['mse']}")
    print(f"git commit: {best_run.data.tags['mlflow.source.git.commit']}")
    init(model_name)
    registered_model = client.get_registered_model(model_name)

    # Create new model version
    source = f"{best_run.info.artifact_uri}/pytorch-model"
